...
const heroImagePath = path.join(__dirname, 'assets', 'home-hero.jpg');
...
const aboutSections = [
  ...
  {
    title: 'Ассортимент продукции Leppa: сантехника для вашей ванной',
  ...
  {
    title: 'Контакты и призыв к действию',
...
const homePage = {
  ...
  benefits: [
    { title: 'Качество', description: 'Продукты соответствуют международным стандартам и проходят контроль качества', icon: 'quality' },
    { title: 'Поддержка', description: 'Наша команда предоставляет профессиональные консультации', icon: 'support' },
    { title: 'Цены', description: 'Лучшие цены на рынке без компромиссов в качестве', icon: 'price' },
    { title: 'Надежность', description: 'Быстрая доставка и стабильный сервис, на который можно положиться', icon: 'delivery' },
  ],
...
async function findSingle(strapi, uid) {
  return (
    await strapi.documents(uid).findFirst({ status: 'published' })
    ?? await strapi.documents(uid).findFirst({ status: 'draft' })
  );
}

async function upsertSingle(strapi, uid, data) {
  const existing = await findSingle(strapi, uid);
  ...
  await strapi.documents(uid).create({ status: 'published', data });
  return 'created';
}

async function ensureHeroImage(strapi) {
  const name = 'leppa-home-hero.jpg';
  ...
  const stats = await fs.promises.stat(heroImagePath);
  const [uploaded] = await strapi.plugin('upload').service('upload').upload({
    data: {
      fileInfo: {
        name,
        alternativeText: 'Премиальная сантехника Leppa & WenSton',
    ...
    files: {
      ...
      mimetype: mime.lookup(heroImagePath) || 'image/jpeg',
      size: stats.size,
    },
  });
...
async function main() {
  ...
  try {
    ...
    const heroImage = await ensureHeroImage(app);
    const siteResult = await upsertSingle(app, SITE_SETTING_UID, siteSettings);
    const homeResult = await upsertSingle(app, HOME_PAGE_UID, {
      ...homePage,
      heroImage: heroImage.id,
    });

    console.log(`[done] site-setting=${siteResult} home-page=${homeResult} hero=${heroImage.name}`);
  } finally {
    if (app) {
      await app.destroy();
    }
